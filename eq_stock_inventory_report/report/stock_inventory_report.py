...
class eq_stock_inventory_report_stock_inventory_report(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
           ...
           'get_stock_move_by_location': self.get_stock_move_by_location
        }
    ...
    def get_stock_move_by_location(self, record, location, product):
        start_date = record.start_date.strftime("%Y-%m-%d")
        end_date = record.end_date.strftime("%Y-%m-%d")
        
        #values = self.env['stock.move']
        
        _logger.info('LA DATE DE DEBUT ' +str(start_date))
        _logger.info('LA DATE DE FIN ' +str(end_date))
        self._cr.execute('''
            SELECT sm.id
            FROM stock_move sm
            WHERE sm.date >= %s and sm.date <= %s and sm.state = 'done' and product_id = %s
        ''', (start_date, end_date, product.id))
        move_ids = [x[0] for x in self.env.cr.fetchall()]
        _logger.info('LA VALEUR DE MOVE IDS EST DE KHK' + str(move_ids))
#         values = self._cr.dictfetchall()
        #_logger.info('LA VALEUR DE LA lacation EST DE ' + str(location))
        values = [move for move in self.env['stock.move'].browse(move_ids) if move.location_id.id == location.id or move.location_dest_id.id == location.id]
        _logger.info('LA VALEUR DE LA REQUETES EST DE KHK' + str(values))
        return values
